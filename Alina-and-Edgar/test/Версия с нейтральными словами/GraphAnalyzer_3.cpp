...
bool add_vertex(set<int> &Result, set<int> &Opposite, set<int> &Candidate, ofstream& fres, vector<int> &dist, vector<int> &opp_dist)
{
...
void analyze(vector<string> &seed_pos, vector<string> &seed_neg, ofstream& fpos, ofstream& fneg)
{
	...
	for (int i = 0; i < seed_pos.size(); i++)
	{
		if (get_index(seed_pos[i]) != -1)
			push_vertex(Result_pos, Result_neg, Candidate, get_index(seed_pos[i]), pos_dist, fpos);
	}

	for (int i = 0; i < seed_neg.size(); i++)
	{
		if (get_index(seed_neg[i]) != -1)
			push_vertex(Result_neg, Result_pos, Candidate, get_index(seed_neg[i]), neg_dist, fneg);
	}

	int step = 1;
	bool fl_pos = true, fl_neg = true;
    while (Candidate.size() != 0 && (fl_pos || fl_neg))
    {
    	if (step)
    	{
    		fl_pos = add_vertex(Result_pos, Result_neg, Candidate, fpos, pos_dist, neg_dist);
    	}
    	else
    	{
    		fl_neg = add_vertex(Result_neg, Result_pos, Candidate, fneg, neg_dist, pos_dist);
    	}
    	step = 1 - step;
    }  
    fpos.close();
    ...
    ofstream fplot;
    fplot.open("big_test_3/plot_3.csv");
