...
bottom  = length(lvls)
for (i in 0:(length(lvls) - 1)) {
  if (counts[length(lvls) - i, 3] != 0) {
    bottom = length(lvls) - i
    break
  }
}
top = 50
bottom = 95
cfp = counts[top:bottom, ]
plot(0, 0, col = 'white', xlim=c(0, dim(cfp)[1]), ylim = c(cfp[1, 1], cfp[dim(cfp)[1], 1]), main = "pos and neg ratio for levels")
for (i in 1:dim(cfp)[1]) {
  py = cfp[i, 1] * cfp[i, 2] / (cfp[i, 2] + cfp[i, 3])
  rect(i - 1, 0, i, py, col = 'green')
  rect(i - 1, py, i, cfp[i, 1], col='red')
}
for (i in (77:83)) {
  print(cwords[[i]])
}
