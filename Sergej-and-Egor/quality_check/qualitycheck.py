...
Script parameters:
    ...
    7: transformed words file (not necessery)
...
pos_count = len(new_dict)
print(pos_count)
read_neg(new_dict, sys.argv[2])
neg_count = len(new_dict) - pos_count
print(neg_count)
read_neutral(new_dict, sys.argv[3])
neu_count = len(new_dict) - pos_count - neg_count
print(neu_count)
...
for key in new_dict.keys():
    if key in big_dict and big_dict[key] == new_dict[key] and new_dict[key] != 0:
        quality_value += 1
    elif key not in big_dict and key not in big_ext_dict:  # means, that we found a new word or it's transformed
        big_ext_dict[key] = 0

print('Quality value: ' + str(quality_value) + ' => '
      + str(quality_value * 100 / (pos_count + neg_count)) + '%')
